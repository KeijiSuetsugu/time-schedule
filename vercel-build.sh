...
if [ "$VERCEL" = "1" ]; then
  ...
  # Decimal精度を自動修正と本院の場所登録
  echo "🔧 Ensuring decimal precision and adding default location..."
...
(async () => {
  try {
    ...
    if (checkResult.rows.length === 0) {
      await client.query(
        "INSERT INTO \"Location\" (id, name, latitude, longitude, radius, enabled, \"createdAt\", \"updatedAt\") VALUES ('honin-main', '本院', 33.88507161918166, 130.70780362473587, 100, true, NOW(), NOW())"
      );
      console.log('✅ 本院の場所を登録しました (緯度:33.88507161918166, 経度:130.70780362473587)');
    } else {
      await client.query(
        "UPDATE \"Location\" SET latitude = 33.88507161918166, longitude = 130.70780362473587, \"updatedAt\" = NOW() WHERE name = '本院'"
      );
      console.log('✅ 本院の場所を更新しました (緯度:33.88507161918166, 経度:130.70780362473587)');
    }
  } catch (err) {
    console.log('⚠️  DB操作エラー:', err.message ? err.message.substring(0, 100) : err);
  } finally {
    await client.end();
  }
})();
